/**
 * @file backend_glasses_classifier.c
...
static bool match_name(const char *adv_name, size_t adv_name_len,
    ...
    glasses_detection_t *out)
{
    if (mac == NULL || out == NULL ||
        (mfr_data_len > 0 && mfr_data == NULL) ||
        (svc_uuid16_count > 0 && svc_uuid16s == NULL)) {
        return false;
    }
    ...
    for (size_t u = 0; u < svc_uuid16_count; u++) {
    ...
    out->first_seen_ms = observed_ms;
    out->last_seen_ms = observed_ms;

    if (adv_name && adv_name_len > 0) {
        size_t copy_len = adv_name_len < sizeof(out->device_name) - 1
                          ? adv_name_len : sizeof(out->device_name) - 1;
