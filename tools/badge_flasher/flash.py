...
class FlashEngine:
    ...
    def reset_device(self, device: UsbDevice) -> None:
        ...
        if _reported_mac(output) != device.mac:
            raise FlashError("final reset reached a different ESP32 MAC")
    ...
    def prove_device(self, device: UsbDevice) -> None:
        ...
        if _reported_mac(output) != device.mac:
            raise FlashError("pre-write identity proof reached a different ESP32 MAC")
    ...
    def flash_and_verify(
        ...
        if _reported_mac(write_output) != device.mac:
            raise FlashError(f"{role}: esptool wrote a different ESP32 MAC")
        if "Hash of data verified" not in write_output:
            raise FlashError(f"{role}: write verification evidence missing")
        ...
        if _reported_mac(readback_output) != device.mac:
            raise FlashError(f"{role}: readback came from a different ESP32 MAC")
        ...
        if verified_regions != len(layout["parts"]):
            raise FlashError(
                f"{role}: explicit flash readback evidence missing "
