...
Fullsize backend platform, scanners, Android, and host-supplied badge firmware
upload are separate families or follow-on work.
...
Android's current native-badge-only identity gate is intentionally outside
this branch. Android is not declared compatible until a separate change
accepts the exact Lite tuple and keeps cross-family flashing disabled.
