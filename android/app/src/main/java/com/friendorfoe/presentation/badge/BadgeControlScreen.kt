...
fun BadgeControlScreen(
    ...
    val isBackendLite = state.controlStatus?.isBackendLite() == true
    ...
    LazyColumn(
        ...
        if (isBackendLite) {
            item { BadgeLiteHeadlessSection(state) }
        } else {
            item {
...
internal fun BadgeHeader(isBackendLite: Boolean = false) {
    Row(
        ...
        Text(
            text = if (isBackendLite) "Badge Lite" else "Badge",
...
@Composable
private fun BadgeLiteHeadlessSection(state: BadgeUsbState) {
    val status = state.controlStatus ?: return
    BadgeSection(
        title = "Backend Badge Lite",
        subtitle = "Headless USB uplink",
        modifier = Modifier.testTag("badge_lite_headless"),
    ) {
        Text(
            text = "Lite has no LCD. Android keeps its USB live lease acknowledged and shows incoming detections here.",
            style = MaterialTheme.typography.bodySmall,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        Column(
            ...
            BadgeStatusMetricRow("Target", status.firmwareTarget.ifBlank { "Not reported" })
            BadgeStatusMetricRow("Project", status.protocolProject.ifBlank { "Not reported" })
            BadgeStatusMetricRow(
                "Capabilities",
                status.capabilities.sorted().joinToString().ifBlank { "Not reported" },
            )
        }
        Text(
            text = "Display, theme, filter, and firmware-control actions are intentionally unavailable for Lite badges.",
            style = MaterialTheme.typography.bodySmall,
